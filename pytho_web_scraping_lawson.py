...
### let's put it in a shape of a class to use it later
...
############# new version of the modified fn link_crawler. It includes more changes than just to extract the Alexa files
...
AC()
start_time = time()
link_crawler(AC.urls, '$^', cache=RedisCache()) # '$^' - regex to avoid collecting links from each page. If we do want it, then '.'
print('Total time: %ss' % (time() - start_time))

### Total time 1394 s

##############################################################################################
############# Multithreaded links crawler
### I couldn't make it work in jupyter, so check it out as a separate file/module
### speed    ### 378 sec - 4x quicker than the sequential - not bad



##################################################################################
######## Multiprocessing crawler - it needs to move the crawl queue from being in memory (as it is now)
######## to redis so other processes can access it too

######## I can't make the multiprocessing version to run... getting an error from multiprocessing module
